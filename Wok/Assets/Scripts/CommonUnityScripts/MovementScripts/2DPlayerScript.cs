...
public class PlayerController : MonoBehaviour
{
	...
	void FixedUpdate()
	{
		...
		{
			rb.linearVelocity -= new Vector2(rb.linearVelocity.x / 5, 0);
			if (Mathf.Abs(rb.linearVelocity.x) < 0.01)
			{
				rb.linearVelocity = new Vector2 (0, rb.linearVelocity.y);
			}
		}
		
		MainCamera.transform.localPosition = Vector3.MoveTowards(MainCamera.transform.localPosition, new Vector3 (rb.linearVelocity.x/8, 0, -1), 0.25f);
		animator.SetFloat("Hor", rb.linearVelocity.x);
	}
	...
	private void OnTriggerEnter2D(Collider2D collision)
	{
		...
		{
			rb.linearVelocity = new Vector2(rb.linearVelocity.x * -2, 2);
			Debug.Log("Bounce!");
		}
